...
- **Publish status:** version bumped 1.0.6 → 1.0.7, code ready + pushed on branch. **Windows installer NOT built in this session** — it runs in a Linux cloud container (no Wine, no native toolchain), so `build:win` + launch-verify the packaged exe must still happen on a Windows PC (or CI). Auto-update reaches aunt only once a non-draft GitHub Release `v1.0.7` is published with the dashed exe + `.blockmap` + `latest.yml`.
